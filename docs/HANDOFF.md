...
| **G3** | the store outlives the run; population B born into population A's record | **NOT MET, and the campaign says why.** At six balanced seeds every test sits between t = −1.5 and t = +1.3: content −0.058 ± 0.041, alignment contrast −0.100 ± 0.066, stale-mark ratio t = +0.68, preparation hit t = +0.63. The signs are consistent (aligned content negative in 5 of 6) and nothing is significant. **The design is powered to almost exactly the effect it measures** — per-seed sd 0.100 means twelve seeds detect 0.058, and the effect is 0.058 — so it will read marginal either way; `docs/G3_WRITEUP.md` §5.6b. That, and not seed luck, explains a statistic that reached t = −3.38 at five seeds and t = −1.42 at six. Two defects in the acceptance are recorded separately (§5.6, §5.6a) and neither rescues the claim. Earlier states of this row, now superseded: | The corroboration campaign completed and `acceptance` returns false at B§5.2's own bar. Seeds 1 and 2 fail the coherence criteria (the two information-removing controls do not land together); content moves the right way in 3/3. Aligned content −0.131 / −0.005 / −0.051, mean **−0.062**, sd 0.064; misaligned +0.018 / +0.018 / −0.021, mean +0.005. The **alignment contrast is what survives** — negative in all three aligned seeds, centred on zero misaligned, separation 1.7 se — but 3/3 negative is p = 0.125 by sign test and nothing here is significant. `docs/G3_WRITEUP.md` §5.2–5.4. Previously recorded as MET at one seed, on the project owner's explicit decision to relax B§5.2's 3/3 bar; `min_seeds` is a recorded parameter of `acceptance`, not a hard-coded constant, and `docs/G3_WRITEUP.md` §4.4 records the basis. Seed 0 aligned: content **−0.131**, reading **−0.119**, the two information-removing controls agreeing to 0.012 nfc and 0.04 on the stale ratio, Gate R PASS (z −0.68, 3 epochs). Seed 0 **misaligned: +0.018** — the record helps only when it is true of B's world, which is the sharpest internal check in the milestone and one no control arm can supply. |
| G4 | a world that hardens, one mechanic per milestone | **mechanic 1's number is dominated by seed variance — it does not measure hardening.** Changing nothing but the seed shifts content by +0.126; mechanic 1's whole reported shift is +0.166 and the economics diagnostic's is +0.122. The between-seed spread at a fixed world is ~0.13, which is the entire magnitude of the G3 effect. `docs/G4_WRITEUP.md` §2.3–2.5 withdraws the confound finding and keeps it as a hypothesis. What the run reported: Content went −0.131 (K=5) → **+0.035** (K=7): compounding **+0.166**, the wrong sign, and the effect does not shrink so much as vanish. The diagnosis is in `docs/G4_WRITEUP.md` and it is a confound in the mechanic, not a null result about B§2 — `fresh store` is *better* in the "harder" world (hit 0.474 → 0.544 against a chance that fell 0.200 → 0.143; nfc 2.035 → 1.678), so the world was not hardened. EV-neutrality forces `prep_value` from 1.0 to 1.5 when K goes 5 → 7, feeding the genetic channel v3.11 found dominant. A diagnostic control at K=5 with the same payoff scale is running. Mechanic 1 built: `world_at_k` + `check_hardened_world` (a hardened world may move only what a mechanic declares, and the chance EV of a preparation must still be zero); the engine is K-parameterised as version `G4-k` and is **bit-identical at K = 5** — 32 rows × 133 fields with zero differing, and a full-length succession reproducing seed 0 to the digit. Mechanics 2 and 3 are deliberately unspecified: `docs/G4_SPEC.md` §2–3 settle their design questions against mechanic 1's numbers rather than in advance of them. |
...
0. **Decide whether to reduce the per-seed spread before buying more seeds.** This is the
   decision that governs everything else and it is the project owner's. Seeds reduce the error
   only as √n — 12 to 48 buys a factor of two for four times the compute — while the per-seed
   spread (sd 0.100) is set by design parameters: B's length, the population size, the width of
   the claim window. Changing any of them changes what is being measured and needs its own
   justification, which is exactly why it should be decided rather than drifted into. §5.6b has
   the numbers.
0b. **G3's gate is not met and the milestone is reopened.** 1 of 3 seeds. The honest options are
...
2. **Two engine changes are written and staged, unapplied**, in `docs/patches/`. Both are additive
   ...
   * `apply-g5-policy-hook.py` — G5-D7's `external_policy`, then §7.1's equivalence check, which
     needs no provider.
3. **Two findings that are not on any claim line** and would each need their own measurement:
   * **A record disperses competence as well as shifting it.** Eight replicates per arm: the arm
     that can read the record has sd 0.144, the arm that cannot has 0.088 (variance ratio 2.65 on
     7 and 7 df, F ≈ 0.11). Suggestive only, and nothing in B§2 asks about dispersion.
   * **The K=7 content effect is positive in both seeds run so far** (+0.035, +0.194) where the
     K=5 baseline is negative in both (−0.131, −0.005), giving compounding +0.166 and +0.199 —
     consistent in sign and size across two seeds. Read it against `docs/G4_WRITEUP.md` §2.1's
     confound before reading it as hardening: the K=7 world also pays 50% more for a correct
     preparation, and its population is better without any record at all. The
   ...
   Mechanics 2 and 3 in particular should not be built against mechanic 1's numbers, which was
   `docs/G4_SPEC.md`'s plan and is no longer sound.
2. ~~**G4 mechanic 1's number**~~ — done, and superseded by items 0 and 1. The run is against the *stored* G3 seed-0
