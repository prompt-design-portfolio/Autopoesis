...
3. **Sandbox on Colab.** Containers are unavailable in a Colab runtime, so the hardened backend and
   ...
   under isolation.
4. **Levels 1 and 2 of §65 were one number until M13.** "External knowledge is useful" and "a
   newcomer benefits from a matured environment" are different claims and were measured with a
   single comparison. Level 1 now runs against `solo_in_mature` — a blinded agent facing the same
   matured workspace — and Level 2 against `baseline_empty`. They currently agree at +0.300, and
   the agreement is evidence that arm enforcement holds rather than a reason to re-merge them.
5. **Reading must not write.** The engine opens every transaction with `BEGIN IMMEDIATE` so the
...
6. **Does the newcomer advantage generalise?** A result measured on one task family is a result
