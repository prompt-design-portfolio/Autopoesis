#!/usr/bin/env python3
"""Build autopoiesis_v3_8_staged_growth.ipynb.  Colab-only: sim.py and analysis.py uploaded
alongside, no local paths, no multiprocessing, no run script."""
import json, pathlib

HERE = pathlib.Path(__file__).parent
MD_INTRO = (HERE / "notebook_intro_v3_8.md").read_text()
MD_READING = (HERE / "notebook_reading_v3_8.md").read_text()
...
print("observation size:", sim.N_IN, " hidden:", A.WORLD["hidden"],
      " chance recipe hit:", round(A.CHANCE, 3))
...
# To resume:  import pickle; results = pickle.load(open("results_v3_8.pkl", "rb"))
results = A.run_experiment(seeds=SEEDS, phase_steps=PHASE_STEPS,
                           save_path="results_v3_8.pkl", **OVERRIDES)
'''
...
nb = {
    "cells": [
        ...
        md("## 2. Run\n\n**Runtime.** A staged run is 16000 steps — twice a v3.6 run — but populations "
           ...
           "\n\n```python\nimport pickle\nbase = pickle.load(open(\"results_v3_8.pkl\", \"rb\"))\n"
    ...
    "metadata": {
        "colab": {"name": "autopoiesis_v3_8_staged_growth.ipynb", "provenance": []},
...
out = HERE / "autopoiesis_v3_8_staged_growth.ipynb"
