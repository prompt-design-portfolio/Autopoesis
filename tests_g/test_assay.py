...
def cheap_record():
    """A record built by driving the engine's own `write_mark`, with no full run behind it.

    A1.6 is satisfied -- this is the mechanism, not a mock -- and it costs milliseconds, so the
    tests that are about the *shape* of the store conditions do not have to pay for a run. Only
    the tests that need an actual replay use `run_and_record` below, and those are marked slow.
    """
    import sim_v3_13
    ...
    return record_from_world(world, arm="collective", t=700, era_index=0,
                             engine_sha256="0" * 64, cfg_digest="test")


@pytest.fixture(scope="module")
def run_and_record():
    """A real 800-step run and the record it left. Slow: it is a run of the engine."""
...
def test_the_store_conditions_differ_only_in_what_the_population_starts_with():
...
def test_hidden_starts_empty_and_permuted_starts_relabelled(cheap_record):
...
def test_the_permuted_arm_uses_a_global_permutation_not_the_per_cell_control(cheap_record):
