#!/usr/bin/env python3
"""Build autopoiesis_v3_12_preparation_world.ipynb.  Colab-only: sim.py and analysis.py uploaded
alongside, no local paths, no multiprocessing, no run script."""
import json, pathlib
from nbcheck import write_checked

HERE = pathlib.Path(__file__).parent
MD_INTRO = (HERE / "spec_v3_12.md").read_text()
MD_READING = (HERE / "notebook_reading_v3_12.md").read_text()
...
CKPT = "results_v3_12.pkl"
CORE = ["fixed", "scrambled", "plastic (W2)"]
ALL  = list(A.VARIANTS)

# REFRESH: (arm, seed) pairs to re-run even though the checkpoint has them.  Fill from the audit
# below if a checkpoint ever spans sim builds.  v3.12 starts from an EMPTY checkpoint, so this is
# empty and stays empty unless a mid-grid sim change forces it.
REFRESH = []

if MODE == "quick":
    SEEDS, PHASE_STEPS, ARMS, OVERRIDES = [0], 1500, CORE, dict(prep_every=300, recipe_every=500)
    CKPT = "results_v3_12_quick.pkl"
...
nb = {
    ...
    "metadata": {
        "colab": {"name": "autopoiesis_v3_12_preparation_world.ipynb", "provenance": []},
...
write_checked(nb, HERE / "autopoiesis_v3_12_preparation_world.ipynb")
